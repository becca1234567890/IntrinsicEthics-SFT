"""
IntrinsicEthics-SFT — Step 02: Build Training Data

Two-phase pipeline. Phase 1 sources prompts from human-curated benchmarks
(HarmBench, JailbreakBench, XSTest, Dolly, OR-Bench), deduplicates with
SBERT (paraphrase-MiniLM-L6-v2, threshold=0.85), and caches pool files.
Phase 2 calls the Anthropic Haiku API once per sourced prompt to generate
the structured reasoning chains for both training conditions (Version B,
Version C). Produces train/test splits per condition. Pool composition
and counts are documented in the repository README.

Inputs:
    Anthropic API (Haiku)
    HuggingFace datasets (HarmBench, JailbreakBench, XSTest, Dolly, OR-Bench)

Outputs:
    /content/drive/MyDrive/ethics_experiment/data/raw/sourced_prompts_*.jsonl
...
Runtime:
    L4 | ~3 hr | ~$3 in Haiku API credits

Configuration:
    DRY_RUN (top of file): True for fast smoke test, False for full run
"""

# ┌─────────────────────────────────────────────────────────────────────────────┐
# │ CONFIGURATION — edit these before running                                   │
# │                                                                             │
# │ ⚠  TO REGENERATE FROM SCRATCH — delete these files from Drive first:       │
# │      data/processed/train_condition_b.jsonl                                 │
# │      data/processed/train_condition_c.jsonl                                 │
# │      data/raw/all_scenarios.jsonl                                           │
# │      data/raw/progress.json                                                 │
# │      data/raw/sourced_prompts_*.jsonl   (prompt pool caches)                │
# └─────────────────────────────────────────────────────────────────────────────┘
...
# but token is forwarded for consistency with Obsolete/train.py.
...
if _hf_token:
    ...
    print("HF_TOKEN       : NOT FOUND — fine-tuning phase will fail on gated models. "
          "Add it in Colab Secrets before running Obsolete/train.py.")
...
# Identical fence-stripping logic to _parse_judge_json in 04_train_and_evaluate.py —
...
if DRY_RUN:
    ...
    # Keep in sync with _CAUSAL in 03_spotcheck.py — update both if vocabulary changes.
    _CAUSAL_SET_C = {
    ...
    # Keep in sync with _RULE in 03_spotcheck.py — update both if vocabulary changes.
    _RULE_SET_C = {
        "rule", "policy", "prohibited", "not allowed", "guidelines", "safety rules",
        "violates", "forbidden", "restriction", "must not", "cannot", "against",
        "comply", "compliance",
    }
    # Keep in sync with _CAUSAL_WORDS in 03_spotcheck.py — update both if vocabulary changes.
    _CAUSAL_WORDS_B = {
