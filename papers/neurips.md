...
Filter-style models are vulnerable to rephrasing attacks because the filter operates on surface features of the request. Causal reasoning operates on intent and consequence, which are stable under surface rephrasing. Our jailbreak test set includes all three tiers (surface, roleplay, authority injection), and the intrinsic adapter achieves perfect resistance across all three, while the filter adapter fails on 18/28 attempts. This suggests causal reasoning generalizes to adversarial distribution shifts in a way that rule-following does not.\footnote{The research conversation that produced this paper was itself paused by a production filter-style safety intervention mid-session. The conversation concerned AI reasoning failure modes and alignment methodology---a net-positive outcome by any causal trace---but matched surface patterns associated with adversarial content. This is a naturalistic demonstration of the edge case failure mode: a filter operating on surface features misclassified constructive safety research as harmful. A causal consequence-modeling system reasoning about the actual downstream effects of the conversation would not have done so.}
